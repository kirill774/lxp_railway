...
SYSTEM_PROMPT = """Ты — опытный студент, который делает учебные работы аккуратно и по ТЗ.
...
PROMPT_PROFILES = {
    "kt": [
        ...
        "Дай прямой ответ по пунктам ТЗ, без длинного вступления.",
        "Покажи ход решения, если есть расчёты, таблицы, сравнения или выводы.",
        "В конце добавь короткий итог на 2-4 предложения.",
    ],
    "essay": [
        "Тип работы: эссе / реферат.",
        "Сделай логичную структуру: тезис, аргументы, примеры, вывод.",
        "Пиши связными абзацами, не превращай весь текст в список.",
        "Добавь 3-5 источников или ориентиров для списка литературы, если тема это допускает.",
    ],
    "presentation": [
        "Тип работы: презентация.",
        "Дай материал с делением на слайды: номер, заголовок, 3-5 тезисов.",
        "Добавь идеи визуалов: диаграмма, фото, схема, пример макета.",
        "Формулировки должны быть короткими, пригодными для слайдов.",
    ],
    "photoshop": [
        "Тип работы: Adobe Photoshop / визуальный макет.",
        "Опиши концепцию, композицию, слои, цвета, шрифты и порядок выполнения.",
        "Если нужен макет, дай техническое задание для сборки: размер, сетка, элементы, экспорт.",
        "Отдельно перечисли критерии проверки и что должно быть видно в готовом файле.",
    ],
    "smm": [
        "Тип работы: SMM / контент-маркетинг.",
        "Определи аудиторию, цель, tone of voice, площадки и метрики.",
        "Добавь практические элементы: рубрики, посты, контент-план, KPI или примеры текстов.",
        "Пиши как рабочее решение для реального малого бизнеса, а не общую теорию.",
    ],
    "default": [
        "Тип работы: учебное задание.",
        "Сначала закрой требования ТЗ, затем добавь пояснения и примеры.",
        "Если данных не хватает, сделай разумные допущения и явно обозначь их.",
    ],
}

FORMAT_HINTS = {
    "pptx": "Формат результата: материал должен легко раскладываться на слайды.",
    "xlsx": "Формат результата: используй таблицы, списки полей, показатели и формулы где уместно.",
    "txt": "Формат результата: чистый текст без markdown-сложности, но с понятными разделами.",
    "docx": "Формат результата: полноценный документ с заголовками, абзацами и итогом.",
}

def detect_prompt_profile(subject: str, task: str, fmt: str) -> str:
    text = f"{subject}\n{task}\n{fmt}".lower()
    if any(word in text for word in ["презентац", "слайд", "ppt", "pptx"]):
        return "presentation"
    if any(word in text for word in ["photoshop", "фотошоп", "adobe", "макет", "баннер", "визуал"]):
        return "photoshop"
    if any(word in text for word in ["smm", "контент", "пост", "таргет", "соцсет", "контент-план"]):
        return "smm"
    if any(word in text for word in ["эссе", "реферат", "essay"]):
        return "essay"
    if any(word in text for word in ["кт", "контрольная точка", "тест", "контрольн"]):
        return "kt"
    return "default"
...
def build_prompt(job: dict, lxp_tz: str = "") -> str:
    ...
    profile = detect_prompt_profile(subject, task, fmt)
    ...
    parts = [
        ...
        f"Формат вывода: {fmt}",
        f"Профиль задания: {profile}",
        FORMAT_HINTS.get(fmt, FORMAT_HINTS["docx"]),
        "",
        "Инструкции по выполнению:",
        *[f"- {line}" for line in PROMPT_PROFILES[profile]],
        "",
    ]
